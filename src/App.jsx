...
function App() {
  ...
  const updateJob = (id, field, value) => {
    setJobs(jobs.map(job => {
      if (job.id === id) {
        ...
        if (field === 'status' && value === 'application submitted') {
          updated.applied = 'yes';
        }
        // Feature 2: when status changes to 'accepted', fire confetti!
        if (field === 'status' && value === 'accepted') {
          ...
          // Extra side bursts for a premium celebratory layout
          setTimeout(() => {
            confetti({
              particleCount: 50,
              angle: 60,
  ...
  /**
   * Exports tracked jobs as a JSON file.
   */
  const exportData = (e) => {
    e.preventDefault();
    ...
    link.href = url;
    link.download = `folja-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  /**
   * Triggers file picker to import a JSON backup of tracked jobs.
   */
  const triggerImport = (e) => {
    ...
    fileInput.onchange = (event) => {
      ...
      reader.onload = (fileEvent) => {
        try {
          const importedJobs = JSON.parse(fileEvent.target.result);
          if (Array.isArray(importedJobs)) {
            if (window.confirm("Are you sure you want to import this file? It will replace all currently tracked job applications.")) {
              setJobs(importedJobs);
              alert(`Successfully imported ${importedJobs.length} applications!`);
            }
          } else {
            alert("Invalid file format. Please upload a valid JSON array generated by följa.");
          }
        } catch (err) {
    ...
    <>
      ...
      <main className="app-main">
        ...
        <div className="footer-data-actions">
          <button className="data-action-link" onClick={exportData}>export</button>
          <button className="data-action-link import-btn" onClick={triggerImport}>import</button>
        </div>
      </main>
    </div>
    </>
